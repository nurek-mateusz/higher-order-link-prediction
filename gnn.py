...
def get_data_and_node_mapping(dataset, split_type, mode, n_cores):
        ...
        # Graph
        with open(f'processing_dataset/{split_type}/{dataset}/G_{mode}.pickle', 'rb') as file:
                G = pickle.load(file)
...
if __name__ == "__main__":
    ...
    G_train, edge_index_train, candidates_train, labels_train = get_data_and_node_mapping(dataset, split_type, 'train', n_cores=10)
    G_val, edge_index_val, candidates_val, labels_val = get_data_and_node_mapping(dataset, split_type, 'val', n_cores=10)
    G_test, edge_index_test, candidates_test, labels_test = get_data_and_node_mapping(dataset, split_type, 'test', n_cores=10)
    ...
    def ensure_dir(path):
        if not os.path.exists(path):
            os.makedirs(path)
    results_base = f"results_gnn/{split_type}/{dataset}"
